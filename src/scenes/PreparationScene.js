// класс соответствует состоянию подготовки к матчу
const shipDatas = [
...
class PreparationScene extends Scene {
  ...
  update() {
    ...
    if (!this.draggedShip && mouse.left && !mouse.pLeft) {
      //определяем находится ли мышка над кораблем
    ...
    if (!mouse.left && this.draggedShip) {
      ...
      const { left, top } = ship.div.getBoundingClientRect(); // координаты верхнего левого корабля
      ...
      const cell = player.cells.flat().find((cell) => isUnderPoint(point, cell)); //находится ли ячейка под точкой

      if (cell) {
        //если нашли, то размещаем корабль
  ...
  startComputer(level) {
    ...
    let untouchables = [];
    //ячейки, в которые компьютер стрелять не будет, генерируются в getRandomSeveral
    if (level === 'simple') {
